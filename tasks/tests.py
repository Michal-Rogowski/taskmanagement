...
class TaskModelTest(TestCase):
    ...
    def test_create_and_query_tasks(self):
        Task.objects.create(title="Fix bug", organization=self.org, metadata={"sprint": 21})
        Task.objects.create(title="Write docs", organization=self.org, metadata={"sprint": 22})
    ...
    def test_org_isolation(self):
        ...
        Task.objects.create(title="A1", organization=self.org, metadata={"tag": "x"})
        Task.objects.create(title="B1", organization=other, assigned_to=bob, metadata={"tag": "y"})
        self.assertEqual(Task.objects.filter(organization=self.org).count(), 1)
        self.assertEqual(Task.objects.filter(organization=other).count(), 1)
...
class FindTasksCommandTest(TestCase):
    ...
    def test_find_tasks_command_runs(self):
        Task.objects.create(title="A1", organization=self.org, metadata={"tag": "x", "priority": 3})
        out = StringIO()
